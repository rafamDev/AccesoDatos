...
public class Main {
	...
	private static void setConsultas(Session session) {
	    ...
	    for (Object[] object : listaAlumnos) {
	    	 System.out.println("Id: " + (String)object[0] + 
		...
		Query alumnosDam = session.createQuery("SELECT c FROM Ciclo AS c WHERE DTYPE = :dam");
	    List<Ciclo> listaDam = alumnosDam.setParameter("dam", "dam").getResultList();
	    for (Ciclo ciclo : listaDam) {
	    	System.out.print("Alumno (DAM)-> ");
	    	System.out.print("Id: " + ciclo.getAlumno().getId() + ", ");
	    	System.out.print("Nombre: " + ciclo.getAlumno().getNombre() + ", ");
            System.out.println("Telefono: " + ciclo.getAlumno().getTlfn());
	    }
